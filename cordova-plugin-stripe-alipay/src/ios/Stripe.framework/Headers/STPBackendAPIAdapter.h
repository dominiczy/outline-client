...
/**
 ...
 @see STPCard
 @param completion call this callback when you're done fetching and parsing the above information
 from your backend. For example, `completion(customer, nil)` (if your call succeeds) or
 ...
 */
- (void)selectDefaultCustomerSource:(id<STPSourceProtocol>)source
 ...
 */
- (void)detachSourceFromCustomer:(id<STPSourceProtocol>)source
 ...
 */
- (void)updateCustomerWithShippingAddress:(STPAddress *)shipping
                               completion:(nullable STPErrorBlock)completion;
