...
typedef NS_ENUM(NSInteger, STPSourceFlow) {
  ...
  /**
   Your customer must be redirected to their online banking service (either a website or mobile
   banking app) to approve the payment.
   */
  STPSourceFlowRedirect,

  /**
   Your customer must verify ownership of their account by providing a code that you post to the
   Stripe API for authentication.
   */
