...
typedef NS_ENUM(NSUInteger, STPPaymentStatus) {
  ...
  /**
   The payment failed due to an unforeseen error, such as the user's Internet connection being
   offline.
   */
...
/**
 ...
 @param error         The error that occurred, if any.
 */
typedef void (^STPJSONResponseCompletionBlock)(NSDictionary* __nullable jsonResponse,
...
/**
 ...
 @param error The error returned from the response, or nil if none occurs. @see StripeError.h for
 possible values.
 */
...
/**
 ...
 @param source The Stripe source from the response. Will be nil if an error occurs. @see
 STPSourceProtocol
 @param error The error returned from the response, or nil if none occurs. @see StripeError.h for
 possible values.
 */
typedef void (^STPSourceProtocolCompletionBlock)(id<STPSourceProtocol> __nullable source,
                                                 NSError* __nullable error);

/**
 A callback to be run with a PaymentIntent response from the Stripe API.

 @param paymentIntent The Stripe PaymentIntent from the response. Will be nil if an error occurs.
 @see STPPaymentIntent
 @param error The error returned from the response, or nil if none occurs. @see StripeError.h for
 possible values.
 */
typedef void (^STPPaymentIntentCompletionBlock)(STPPaymentIntent* __nullable paymentIntent,
                                                NSError* __nullable error);
...
/**
 ...
 @param shippingValidationError If the shipping address is invalid, an error describing the issue
 with the address. If no error is given and the address is invalid, the default error message will
 be used.
...
typedef void (^STPShippingMethodsCompletionBlock)(
    STPShippingStatus status, NSError* __nullable shippingValidationError,
    NSArray<PKShippingMethod*>* __nullable shippingMethods,
    PKShippingMethod* __nullable selectedShippingMethod);
...
/**
 ...
 @param customer     The Stripe customer from the response, or nil if an error occurred. @see
 STPCustomer
 @param error        The error returned from the response, or nil if none occurs.
 */
typedef void (^STPCustomerCompletionBlock)(STPCustomer* __nullable customer,
                                           NSError* __nullable error);
