...
/** This view controller contains a credit card entry form that the user can fill out. On
 * submission, it will use the Stripe API to convert the user's card details to a Stripe token. It
 * renders a right bar button item that submits the form, so it must be shown inside a
...
/**
 ...
 sharedConfiguration] theme:[STPTheme defaultTheme]`.
 */
- (instancetype)init;

/**
 Initializes a new `STPAddCardViewController` with the provided configuration and theme. Don't
 forget to set the `delegate` property after initialization.

 @param configuration The configuration to use (this determines the Stripe publishable key to use,
 the required billing address fields, whether or not to use SMS autofill, etc). @see
 STPPaymentConfiguration
 @param theme         The theme to use to inform the view controller's visual appearance. @see
 STPTheme
 */
...
/**
 ...
 to work properly. @see STPAddCardViewControllerDelegate
 */
@property(nonatomic, weak, nullable) id<STPAddCardViewControllerDelegate> delegate;

/**
 You can set this property to pre-fill any information you've already collected from your user. @see
 STPUserInformation.h
 */
@property(nonatomic, strong, nullable) STPUserInformation *prefilledInformation;

/**
 If you're using the token generated from STPAddCardViewController to make a Managed Account, you
 should set this property to the currency that account will use. Otherwise, you should leave it
 empty. For more information, see https://stripe.com/docs/api#create_card_token-card-currency
 */
