...
public class CordovaStripe extends CordovaPlugin {
  ...
  @Override
  public boolean execute(final String action, JSONArray data, CallbackContext callbackContext)
    ...
    if (requestCode == LOAD_PAYMENT_DATA_REQUEST_CODE) {
      switch (resultCode) {
        ...
        case AutoResolveHelper.RESULT_ERROR:
          ...
          googlePayCallbackContext.error(
              "Error occurred while attempting to pay with GooglePay. Error #" + status.toString());
          break;
      }
    }
  }
  ...
  private void createBankAccountToken(
      final JSONObject bankAccount, final CallbackContext callbackContext) {
    try {
      BankAccount bankAccountObject =
          new BankAccount(bankAccount.getString("account_number"), bankAccount.getString("country"),
              bankAccount.getString("currency"), bankAccount.getString("routing_number"));
  ...
  private void validateExpiryDate(
      final Integer expMonth, final Integer expYear, final CallbackContext callbackContext) {
  ...
  private void createSource(
      final int sourceType, final JSONObject params, final CallbackContext callbackContext) {
    ...
    try {
      ...
      String statementDescriptor =
          params.has("statementDescriptor") ? params.getString("statementDescriptor") : "";
      ...
      switch (sourceType) {
        ...
        case 3:
          sourceParams = SourceParams.createSepaDebitParams(
              name, iban, addressLine1, city, postalCode, country);
          break;
        ...
        case 5:
          sourceParams =
              SourceParams.createAlipaySingleUseParams(amount, currency, name, email, returnURL);
          break;
  ...
  private void createAccountToken(final JSONObject params, final CallbackContext callbackContext) {
    try {
      ...
      Token token = stripeInstance.createAccountTokenSynchronous(
          AccountParams.createAccountParams(params.getBoolean("tosShownAndAccepted"), legalEntity));
  ...
  private void initGooglePay(final CallbackContext callbackContext) {
    paymentsClient = Wallet.getPaymentsClient(cordova.getContext(),
        new Wallet.WalletOptions.Builder()
            .setEnvironment(publishableKey == null || publishableKey.contains("test")
    ...
    task.addOnCompleteListener((Task<Boolean> task1) -> {
      try {
  ...
  private void createGooglePayToken(
      String totalPrice, String currencyCode, final CallbackContext callbackContext) {
    ...
    if (finalRequest != null) {
      cordova.getActivity().runOnUiThread(() -> {
        AutoResolveHelper.resolveTask(paymentsClient.loadPaymentData(finalRequest),
