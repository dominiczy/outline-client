...
export namespace CordovaStripe {
    ...
    export type SourceParams = ThreeDeeSecureParams | GiroPayParams | iDEALParams | SEPADebitParams |
        SofortParams | AlipayParams | AlipayReusableParams | P24Params | VisaCheckoutParams;
    ...
    export class CordovaStripePlugin {
        /**
         * Set publishable key
         * @param {string} key
         ...
         */
        static setPublishableKey(
            key: string, success: BlankCallback = NOOP, error: ErrorCallback = NOOP) {
        ...
        static createCardToken(
            creditCard: CardTokenRequest, success: CardTokenCallback = NOOP,
         ...
         * @param {Function} error
         */
        static createBankAccountToken(
            bankAccount: BankAccountTokenRequest, success: BankAccountTokenCallback = NOOP,
            error: ErrorCallback = NOOP) {
        ...
        static validateCardNumber(
            cardNumber, success: (isValid: boolean) => void = NOOP, error: ErrorCallback = NOOP) {
        ...
        static validateExpiryDate(
            expMonth: number, expYear: number, success: (isValid: boolean) => void = NOOP,
            error: ErrorCallback = NOOP) {
         ...
         */
        static validateCVC(
            cvc: string, success: (isValid: boolean) => void = NOOP, error: ErrorCallback = NOOP) {
         ...
         */
        static getCardType(
            cardNumber: string | number, success: (type: string) => void = NOOP,
            error: ErrorCallback = NOOP) {
         ...
         * @param {Function} error
         */
        static payWithApplePay(
            options: ApplePayOptions,
            success: (token: TokenResponse, callback: (paymentProcessed: boolean) => void) => void,
            error: ErrorCallback = NOOP) {
            ...
            exec(
                (token: TokenResponse) => {
                    success(token, (paymentProcessed: boolean) => {
                        exec(
                            NOOP, NOOP, 'CordovaStripe', 'finalizeApplePayTransaction',
                            [Boolean(paymentProcessed)]);
                    });
                },
                error, 'CordovaStripe', 'initializeApplePayTransaction',
                [options.merchantId, options.country, options.currency, options.items])
        }
        ...
        static payWithGooglePay(
            options: GooglePayOptions, success: (token: TokenResponse) => void,
            error: ErrorCallback = NOOP) {
            exec(
                success, error, 'CordovaStripe', 'payWithGooglePay',
                [options.amount, options.currencyCode]);
        }

        static createSource(
            type: SourceType, params: SourceParams, success: (token: TokenResponse) => void = NOOP,
            error: ErrorCallback = NOOP) {
            exec(
                success, error, 'CordovaStripe', 'createSource',
                [SourceTypeArray.indexOf(type.toLowerCase() as SourceType), params]);
        }
        ...
        static createAccountToken(
            accountParams: AccountParams, success = NOOP, error: ErrorCallback = NOOP) {
