...
const autoLaunch = require("auto-launch"); // tslint:disable-line
const connectivity = require("./connectivity");
const errors = require("../www/model/errors");
const connection_store_1 = require("./connection_store");
...
const connectionStore = new connection_store_1.ConnectionStore(electron_1.app.getPath('userData'));
...
function createWindow(connectionAtShutdown) {
    // Create the browser window.
    mainWindow = new electron_1.BrowserWindow({ width: 360, height: 640, resizable: false });
    const pathToIndexHtml = path.join(electron_1.app.getAppPath(), 'www', 'electron_index.html');
    ...
    const minimizeWindowToTray = (event) => {
    ...
    mainWindow.webContents.on('did-finish-load', () => {
        ...
        if (connectionAtShutdown) {
            ...
            sendConnectionStatus(2 /* RECONNECTING */, connectionAtShutdown.id);
...
function createTrayIcon(status) {
    ...
    const menuTemplate = [
        { label: statusString, enabled: false }, { type: 'separator' },
        { label: quitString, click: quitApp }
    ];
    tray.setContextMenu(electron_1.Menu.buildFromTemplate(menuTemplate));
}
function createTrayIconImage(imageName) {
    const image = electron_1.nativeImage.createFromPath(path.join(electron_1.app.getAppPath(), 'resources', 'tray', imageName));
    if (image.isEmpty()) {
...
const isSecondInstance = electron_1.app.makeSingleInstance((argv, workingDirectory) => {
...
electron_1.app.setAsDefaultProtocolClient('ss');
function interceptShadowsocksLink(argv) {
    if (argv.length > 1) {
        const protocol = 'ss://';
        let url = argv[1];
...
electron_1.app.on('ready', () => {
    ...
    if (isLinux) {
        ...
        electron_1.app.setLoginItemSettings({ openAtLogin: true, args: ["--autostart" /* AUTOSTART */] });
    }
    // TODO: --autostart is never set on Linux, what can we do?
    if (process.argv.includes("--autostart" /* AUTOSTART */)) {
        connectionStore.load()
            ...
            console.log(`${"--autostart" /* AUTOSTART */} was passed but we were not connected at shutdown - exiting`);
            electron_1.app.quit();
        });
    }
    else {
...
promiseIpc.on('is-reachable', (config) => {
    return connectivity
...
async function startVpn(config, id, isAutoConnect = false) {
    if (currentConnection) {
        throw new Error('already connected');
    }
    currentConnection = new process_manager_1.ConnectionManager(config, isAutoConnect);
    currentConnection.onceStopped.then(() => {
        console.log(`disconnected from ${id}`);
        currentConnection = undefined;
        sendConnectionStatus(1 /* DISCONNECTED */, id);
    });
    currentConnection.onReconnecting = () => {
        console.log(`reconnecting to ${id}`);
        sendConnectionStatus(2 /* RECONNECTING */, id);
    };
    currentConnection.onReconnected = () => {
        console.log(`reconnected to ${id}`);
        sendConnectionStatus(0 /* CONNECTED */, id);
    };
...
function sendConnectionStatus(status, connectionId) {
    let statusString;
    switch (status) {
        case 0 /* CONNECTED */:
...
promiseIpc.on('start-proxying', async (args) => {
...
electron_1.ipcMain.on('environment-info', (event, info) => {
...
electron_1.ipcMain.on('quit-app', quitApp);
electron_1.ipcMain.on('localizationResponse', (event, localizationResult) => {
    if (!!localizationResult) {
        localizedStrings = localizationResult;
    }
    createTrayIcon(1 /* DISCONNECTED */);
});
